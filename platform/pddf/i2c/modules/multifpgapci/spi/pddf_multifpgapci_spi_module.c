			   ...
			   const char *buf, size_t count)
{
	...
	unsigned long controller_start =
		spi_start +
		(index * spi_drvdata->temp_sysfs_vals.virt_spi_controller_size);
	pddf_dbg(
		MULTIFPGA,
		KERN_INFO
		"%s: fpga_data_base_addr: 0x%08lx spi_start 0x%08lx controller_start 0x%08lx\n",
		__FUNCTION__, spi_drvdata->bar_start, spi_start,
		controller_start);

	struct spi_board_info *sbi = kzalloc(sizeof(*sbi), GFP_KERNEL);
	if (!sbi)
		return -ENOMEM;
	...
	sbi->chip_select = spi_drvdata->temp_sysfs_vals.spi_cs;
	strcpy(sbi->modalias, spi_drvdata->temp_sysfs_vals.spi_dev_driver);
	...
	res.start = controller_start;
	res.end = controller_start +
	...
	pddf_dbg(MULTIFPGA, KERN_INFO "[%s] platform dev %d registered.\n",
		 __FUNCTION__, index);
	return count;
}
					...
					struct kobject *kobj)
{
	...
	PDDF_DATA_ATTR(
		spi_cs, S_IWUSR | S_IRUGO, show_pddf_data, store_pddf_data,
		PDDF_UINT32, sizeof(uint32_t),
		(void *)&spi_drvdata->temp_sysfs_vals.spi_cs, NULL);
	PDDF_DATA_ATTR(
		spi_driver, S_IWUSR | S_IRUGO, show_pddf_data, store_pddf_data,
		PDDF_CHAR, NAME_SIZE,
		(void *)spi_drvdata->temp_sysfs_vals.spi_driver, NULL);
	PDDF_DATA_ATTR(
		spi_dev_driver, S_IWUSR | S_IRUGO, show_pddf_data,
		store_pddf_data, PDDF_CHAR, NAME_SIZE,
		(void *)spi_drvdata->temp_sysfs_vals.spi_dev_driver, NULL);
	...
	spi_drvdata->attrs.attr_spi_cs = attr_spi_cs;
	spi_drvdata->attrs.attr_spi_driver = attr_spi_driver;
	spi_drvdata->attrs.attr_spi_dev_driver = attr_spi_dev_driver;
	...
	struct attribute *spi_controller_attrs[NUM_SPI_CONTROLLER_ATTRS + 1] = {
		...
		&spi_drvdata->attrs.attr_spi_cs.dev_attr.attr,
		&spi_drvdata->attrs.attr_spi_driver.dev_attr.attr,
		&spi_drvdata->attrs.attr_spi_dev_driver.dev_attr.attr,
		NULL,
	};
					 ...
					 struct kobject *kobj)
{
	...
	kfree(spi_drvdata);
	xa_erase(&spi_drvdata_map, (unsigned long)pci_dev);
}
