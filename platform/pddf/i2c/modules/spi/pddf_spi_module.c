...
ssize_t create_spi_device(struct device *dev, struct device_attribute *da,
  ...
  // echo "$device_name $spi_controller_name $modalias $max_speed_hz $chip_select" >
  // create_spi_device
  int sscanf_result =
  ...
  if (controller == NULL) {
    printk("%s: [%s] Unable to get the spi controller spi_controller_name: %s "
           "for device_name: %s\n",
