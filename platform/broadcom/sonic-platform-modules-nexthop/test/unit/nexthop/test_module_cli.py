...
class TestListCommand:
    def test_lists_pddf_and_custom_modules(self, module_cli_module, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda: {"pddf_led_module": [], "nh_pmbus_core": ["isl68137"]},
        )
        ...
        assert "isl68137" in result.output
...
class TestReloadCommand:
    def test_success(self, module_cli_module, monkeypatch):
        ...
        runner = CliRunner()
        result = runner.invoke(module_cli_module.reload, ["isl68137"])

        assert result.exit_code == 0
        assert calls == ["isl68137"]
    ...
    def test_recreate_devices_flags_are_rejected(self, module_cli_module):
        ...
        for flag in ("--recreate-devices", "--no-recreate-devices"):
            result = runner.invoke(module_cli_module.reload, ["isl68137", flag])
    ...
    def test_hyphenated_module_name_matches_underscore_declaration(self, module_cli_module, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda: {"PLATFORM": {"custom_kos": ["nh_adm1266"]}},
        )
        calls = []
        monkeypatch.setattr(
            "nexthop.module_cli.reload_module",
        ...
        runner = CliRunner()
        result = runner.invoke(module_cli_module.reload, ["nh-adm1266"])

        assert result.exit_code == 0
        assert calls == ["nh-adm1266"]
...
class TestNoUnloadCommand:
    def test_unload_is_not_exposed(self, module_cli_module):
        ...
        runner = CliRunner()
        result = runner.invoke(module_cli_module.cli, ["unload", "isl68137"])
