...
def decode_dpm_fault(dpm_table: Dict[int, str], dpm_bits: Dict[int, int],
                     pdio_in: int) -> str:
    """
    Decode DPM fault code from PDIO input bits.

    Args:
        dpm_table: Mapping of fault codes to descriptions
        dpm_bits: Mapping of PDIO bits to fault positions
        pdio_in: PDIO input bit mask

    Returns:
        DPM fault description string, empty if no fault
    """
    code = 0
    for pdio_bit, fault_pos in dpm_bits.items():
        code |= ((pdio_in >> (pdio_bit - 1)) & 1) << fault_pos
    return dpm_table.get(code, f"code={code}") if code != 0 else ""  # empty string for zero

def decode_power_fault_cause(pdio_input_to_fault_cause:  Dict[int, Dict[str, str]],
                             pdio_in: int) -> (str, str, str, str):
    ...
    for idx, cause_dict in pdio_input_to_fault_cause.items():
        if (pdio_in >> (idx - 1)) & 1:
            return cause_dict.get("hw_cause"), cause_dict.get("hw_desc"),\
                    cause_dict.get("summary"), cause_dict.get("reboot_cause")
    return "", "", "", ""
...
class Adm1266:
    ...
    def get_blackbox_records(self) -> List[Dict]:
        ...
        for fault in faults:
            ...
            dpm = decode_dpm_fault(self.platform_spec.get_dpm_table(),
                                   self.platform_spec.get_dpm_signals(),
                                   record['pdio_in'])
            if dpm:
            ...
            hw_cause, hw_desc, summary, reboot_cause = \
                decode_power_fault_cause(self.platform_spec.get_pdio_input_to_fault_cause(),
                                         record['pdio_in'])
            if hw_cause:
