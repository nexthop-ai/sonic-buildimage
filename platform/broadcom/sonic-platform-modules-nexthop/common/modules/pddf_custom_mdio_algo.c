...
static void fpga_mdio_fixed_delay(void)
{
	// We won't need this once we add a busy bit to the MDIO controller.
	udelay(45);
}

static void fpga_mdio_do_transaction(struct fpga_mdio_priv *priv, int phy_addr,
...
int fpga_mdio_read(struct mii_bus *bus, int phy_addr, int reg_num)
{
	...
	fpga_mdio_do_transaction(priv, phy_addr, dev_addr,
