...
* [<strong>Redis and Json Schema</strong>](#redis-and-json-schema)
  * [ACL and Mirroring](#acl-and-mirroring)
  * [BGP BBR](#bgp-bbr)
  * [BGP Aggregate Address](#bgp-aggregate-address)
  * [ASIC SDK health event](#asic-sdk-health-event)
  * [BGP Device Global](#bgp-device-global)
...
***ACL rule UDF match configuration example***

A rule's match on a user-defined field is its own `ACL_RULE_UDF` entry, keyed by
the ACL table, the rule, and the UDF group name — one entry per group a rule
matches on. The group must be defined in `UDF` and must appear in the `MATCHES`
of the `ACL_TABLE_TYPE` the rule's table uses.

Keeping the group name in a key rather than as a field of the rule's own row is
what lets the rule's field set stay closed: an operator-chosen name and the
schema's declared match fields would otherwise share one token slot in the same
Redis hash.
...
`VALUE` is `<value>/<mask>` in hex. orchagent pads or truncates it to the group's
declared `length`, so its width here does not have to match that length.
...
The **BGP_AGGREGATE_ADDRESS** table contains BGP aggregate address configuration.
...
```json
{
    "BGP_AGGREGATE_ADDRESS": {
        "192.168.0.0/24": {
            ...
            "contributing-address-prefix-list": "AGP_CONTRIBUTING_ROUTES_V4",
            "origin": "igp"
        },
        "fc00::/63": {
            "bbr-required": "false",
...
**Attributes:**
- **bbr-required**: Boolean indicating if BBR (Best path-selection Route) is required for generating aggregate address (default: false)
- **summary-only**: Boolean to only advertise the summary of aggregate address (default: false)
- **as-set**: Boolean to include the AS set when advertising the aggregated address (default: false)
- **aggregate-address-prefix-list**: Optional prefix list name for aggregate address filtering
- **contributing-address-prefix-list**: Optional prefix list name for contributing address filtering
- **origin**: BGP origin code for the aggregate route. Possible values:
  - **igp** (0): Interior Gateway Protocol
  - **egp** (1): Exterior Gateway Protocol
  - **incomplete** (2): Incomplete
  - **unspecified** (255): Unspecified - use default FRR behavior (default)
...
The **CLI** table contains system-wide CLI configuration settings. The
`prompt-format` field defines the bash prompt format using an ordered list
of components that determine what information is displayed in the shell prompt.

```
{
"CLI": {
    "default": {
        "prompt-format": "time,user,host,dir"
    }
  }
}
```
...
The `prompt-format` field accepts a comma-separated list of the following components:

| Component   | Description                          | Example Output        |
|-------------|--------------------------------------|-----------------------|
| user        | Current username                     | admin                 |
| host        | Short hostname                       | sonic-switch          |
| fqdn        | Fully qualified domain name          | sonic-switch.lab.net  |
| time        | Current time (HH:MM:SS)              | 14:30:45              |
| date        | Current date (YYYY-MM-DD)            | 2024-01-15            |
| dir         | Current working directory            | /home/admin           |
...
```
This produces a prompt like: `admin@sonic-switch:/home/admin$`
...
```
This produces a prompt like: `[14:30:45] admin@sonic-switch:/home/admin$`

**CLI Commands:**

- `config cli prompt format <component>...` - Set system-wide prompt format
- `config cli prompt reset` - Reset to default prompt format
- `show cli prompt` - Display current prompt configuration

**Per-User Prompt Configuration:**

Users can have individual prompt settings stored in the **LOCAL_USER** table
via the `cli_prompt` field. When a user has a custom prompt configured, it
overrides the system-wide prompt setting.
...
"LOCAL_USER": {
    "admin": {
        ...
        "password_hash": "...",
        "enabled": "true",
        "cli_prompt": "time,user,fqdn,dir"
    }
  }
}
```

- `config user cli-prompt format <username> <component>...` - Set user's prompt
- `config user cli-prompt reset <username>` - Reset user to system default
...
The **NEIGH_CACHE_TIMER** table configures Linux kernel ARP/ND neighbor cache aging
parameters. Each row is keyed by `scope|family`. Only the `global` scope is
currently supported (per-interface scopes are reserved for a future revision);
`family` is `IPv4` or `IPv6`. Values are applied by `neighsyncd` to every
per-interface neigh directory under `/proc/sys/net/{ipv4,ipv6}/neigh/`. The
optional `gc_thresh*_pct` fields override the default ratios neighsyncd uses
when sizing the kernel `gc_thresh1/2/3` values from CRM neighbor capacity.

```
{
    "NEIGH_CACHE_TIMER": {
        "global|IPv4": {
            "reachable_time_ms": "1800000",
            "gc_stale_time": "60",
            "gc_interval": "30",
            "gc_thresh1_pct": "25",
            "gc_thresh2_pct": "50",
...
|---|---|---|---|
| `scope` | enumeration | `global` | Configuration scope (list key). `global` applies system-wide. Per-interface scopes are reserved for a future revision. |
| `family` | enumeration | `IPv4` / `IPv6` | Address family (list key). |
| `reachable_time_ms` | uint32 | 1000 – 3600000 | Time in milliseconds a neighbor remains in REACHABLE state after a positive confirmation. Maps to `base_reachable_time_ms` sysctl. |
| `gc_stale_time` | uint32 | 1 – 86400 | Seconds before a stale neighbor entry is garbage-collected. Maps to `gc_stale_time` sysctl. |
| `gc_interval` | uint32 | 1 – 86400 | Seconds between neighbor GC runs. Maps to `gc_interval` sysctl. |
| `gc_thresh1_pct` | uint8 | 1 – 100 | Optional. `gc_thresh1` as a percentage of ASIC neighbor capacity (CRM used + available). Default 25 percent. All three `gc_thresh*_pct` fields must be set together or all left unset; partial configuration is rejected. Must be less than `gc_thresh2_pct`. |
| `gc_thresh2_pct` | uint8 | 1 – 100 | Optional. `gc_thresh2` as a percentage of ASIC neighbor capacity. Default 50 percent. All three `gc_thresh*_pct` fields must be set together. Must be greater than `gc_thresh1_pct` and less than `gc_thresh3_pct`. |
| `gc_thresh3_pct` | uint8 | 1 – 100 | Optional. `gc_thresh3` as a percentage of ASIC neighbor capacity. Default 100 percent. All three `gc_thresh*_pct` fields must be set together. Must be greater than `gc_thresh2_pct`. |

### NEXTHOP_TRACKING

The **NEXTHOP_TRACKING** table configures nexthop tracking and route resolution
settings per VRF and address family. Each row is keyed by `vrf_name|afi`.
`vrf_name` is `default` or a user-defined VRF name from the **VRF** table;
`afi` is `ipv4` or `ipv6`. These settings correspond to FRR zebra NHT
configuration such as `ip nht arp-tracking` / `ipv6 nht nd-tracking` and
`ip nht resolve-via-default` / `ipv6 nht resolve-via-default`.

If a `(vrf_name, afi)` entry is not present, `resolve_via_default` is treated
as enabled. This preserves the historical SONiC default used for BGPMON and
similar non-direct IBGP peers.
...
| Field | Type | Description |
|---|---|---|
| `vrf_name` | string | List key. VRF name (`default` or a name from the VRF table). |
| `afi` | enumeration | List key. Address family: `ipv4` or `ipv6`. |
| `neighbor_tracking` | boolean | Optional. When true, nexthops are considered resolved only while the kernel has an active ARP (IPv4) or ND (IPv6) entry for the nexthop address. When false, nexthops are considered resolved as soon as they are routable. Default false. |
| `resolve_via_default` | boolean | Optional. When true, tracked nexthops may be resolved via the default route (`0.0.0.0/0` or `::/0`). When false, tracked nexthops are not resolved through the default route. Default true. |
...
The RA table holds device-wide IPv6 Router Advertisement configuration. The
single `GLOBAL` key carries a `state` field (`enabled` or `disabled`) that
controls whether the device sends RAs on any interface. A missing `RA|GLOBAL`
entry means `enabled`.
...
The **SRV6_MY_SIDS** and **SRV6_MY_LOCATORS** tables introduce Segment Routing over IPv6 configuration.
A locator is a uSID (micro-segment) locator by default; set the optional `full_sid` field to
`"true"` to make it a regular (full-SID) locator instead.
An example is as follows:
```
{
    "SRV6_MY_LOCATORS" : {
...
The **SR_TE_SEGMENT_LIST** and **SR_POLICY** tables introduce Segment Routing Traffic Engineering Policy configuration for SONiC. These tables enable configuration of SR policies that can be used for traffic steering using segment routing.

**Note:** SR-Policy configuration requires the routing daemon's path computation component (`pathd`) to be running in the BGP container. This is automatically started when SONiC is configured with SR-Policy support.

#### SR_TE_SEGMENT_LIST
Defines individual segments that form segment lists. Segments are ordered by their index values.

**Key:** name|index (e.g., "srv6_path|10")

**Attributes:**
- **name**: Segment list name
- **index**: Segment index (uint32, 1-65535) - determines order in the path. Indices don't need to be consecutive, allowing insertion between existing segments (e.g., 10, 20, 30 allows inserting 15)
- **ipv6_address**: SRv6 SID - IPv6 address (Type B segment as defined in RFC 9256). This is currently the only supported segment type.

**Note:** Currently only SRv6 Type B segments (IPv6 addresses) are supported. Future releases will add support for additional segment types including SR-MPLS labels, NAI-based segments (prefix, adjacency, interface-based), and algorithm-based segments.

#### SR_POLICY
The SR_POLICY table uses variable-length keys to represent the hierarchical structure of SR policies in a flat ConfigDB table:

- **2-part key** (color|endpoint): Defines the SR policy itself
- **3-part key** (color|endpoint|preference): Defines a candidate path for the policy
- **4-part key** (color|endpoint|preference|segment_list_name): Associates a segment list with a candidate path

This design is idiomatic SONiC - using key length to distinguish between different configuration levels within a single table.

##### SR Policy Entry (2-part key)
Defines SR policies identified by color and endpoint.

**Key:** color|endpoint (e.g., "100|2001:db8:3::1")

**Attributes:**
- **color**: Policy color/distinguisher (uint32, range 1-4294967295)
- **endpoint**: Policy endpoint IPv4 or IPv6 address
- **name**: Policy name (optional, but must be unique across all policies if specified)

**Note:** Once a policy name is set, it cannot be modified due to backend limitations.

##### Candidate Path Entry (3-part key)
Defines candidate paths for SR policies. Multiple candidate paths can be configured for a single policy with different preferences.

**Key:** color|endpoint|preference (e.g., "100|2001:db8:3::1|100")

**Attributes:**
- **color**: Policy color (must match an existing 2-part SR_POLICY entry)
- **endpoint**: Policy endpoint (must match an existing 2-part SR_POLICY entry)
- **preference**: Candidate path preference (uint32, range 1-4294967295, higher value = higher preference)
- **name**: Candidate path name (required)
- **type**: Path type - currently only "explicit" is supported (default value)

**Note:** Currently only explicit candidate paths are supported. Dynamic paths (computed by PCE or local computation) will be added in future releases.

##### Segment List Association Entry (4-part key)
Associates a segment list with a candidate path. Multiple segment lists can be associated with a single candidate path, each with its own weight, to enable weighted ECMP across the segment lists.

**Key:** color|endpoint|preference|segment_list_name (e.g., "100|2001:db8:3::1|100|srv6_path")

**Attributes:**
- **color**: Policy color (must match parent candidate path)
- **endpoint**: Policy endpoint (must match parent candidate path)
- **preference**: Candidate path preference (must match parent candidate path)
- **name**: Segment list name (must reference an existing SR_TE_SEGMENT_LIST entry)
- **weight**: Optional weight for weighted ECMP when multiple segment lists are configured (uint32, range 1-4294967295, default: 1)

The key components establish the relationship between the candidate path and the segment list.

**Note:** Multiple segment lists may be associated with the same candidate path (color|endpoint|preference) by adding multiple 4-part entries that differ only in the segment list name. Each association can carry its own `weight` for weighted ECMP load balancing across the segment lists.

**Note:** A segment list may be associated with only one candidate path across all SR policies. To move a segment list to another candidate path, delete the old association and add the new association in a single atomic configuration transaction. Management tools that apply individual writes must delete the old association before adding the new one.

**Basic Example - SRv6 Policy:**
```json
{
    "SR_TE_SEGMENT_LIST": {
        "srv6_path|10": {
...
In this example:
- `"100|2001:db8:3::1"` - 2-part key defines the policy with name "static_srv6_policy"
- `"100|2001:db8:3::1|100"` - 3-part key defines candidate path with preference 100 and name "cp1"
- `"100|2001:db8:3::1|100|srv6_path"` - 4-part key associates the segment list "srv6_path" with the candidate path

This configuration generates the following routing configuration:
```
segment-routing
 traffic-eng
  segment-list srv6_path
   index 10 ipv6-address 2001:db8:2::100
   index 20 ipv6-address 2001:db8:3::1
  exit
...
**Multiple Segment Lists Example (Weighted ECMP):**

A candidate path can reference more than one segment list, each with its own weight, to load balance traffic across the segment lists:
```json
{
    "SR_TE_SEGMENT_LIST": {
        "sl1|10": {
    ...
    "SR_POLICY": {
        "100|2001:db8:3::1": {
            "name": "weighted_ecmp_policy"
        },
        "100|2001:db8:3::1|100": {
            "name": "cp1",
            "type": "explicit"
        },
        "100|2001:db8:3::1|100|sl1": {
            "weight": "10"
        },
        "100|2001:db8:3::1|100|sl2": {
            "weight": "20"
        }
    }
}
...
segment-routing
 traffic-eng
  segment-list sl1
   index 10 ipv6-address 2001:db8:1::1
  exit
  segment-list sl2
   index 10 ipv6-address 2001:db8:2::1
  exit
  policy color 100 endpoint 2001:db8:3::1
   name weighted_ecmp_policy
   candidate-path preference 100 name cp1 explicit
...
**Inserting Segments Example:**

Initial configuration with segments at index 10 and 30:
```json
{
    "SR_TE_SEGMENT_LIST": {
        "path1|10": {
...
To insert a segment between index 10 and 30, simply add:
```json
{
    "SR_TE_SEGMENT_LIST": {
        "path1|20": {
...
The resulting segment list will have segments in order: 10, 20, 30.

**Configuration Order:**
1. Configure individual segments (SR_TE_SEGMENT_LIST entries with name|index keys)
2. Configure SR_POLICY entries in order:
   - First, 2-part keys (policies)
   - Then, 3-part keys (candidate paths)
   - Finally, 4-part keys (segment list associations)

**Multiple Candidate Paths Example:**
```json
{
    "SR_TE_SEGMENT_LIST": {
        "primary_path|10": {
        ...
        "backup_path|10": {
            "ipv6_address": "2001:db8:4::4"
        },
        "backup_path|20": {
            "ipv6_address": "2001:db8:5::5"
        }
    },
    "SR_POLICY": {
        "200|2001:db8:10::1": {
            "name": "multi_path_policy"
        },
        "200|2001:db8:10::1|200": {
            "name": "primary_cp",
            "type": "explicit"
        },
        "200|2001:db8:10::1|100": {
            "name": "backup_cp",
            "type": "explicit"
        },
        "200|2001:db8:10::1|200|primary_path": {
        },
        "200|2001:db8:10::1|100|backup_path": {
        }
    }
}
```

In this example, the policy has two candidate paths with different preferences:
- Primary path (preference 200) uses segment list "primary_path"
- Backup path (preference 100) uses segment list "backup_path"
- Higher preference (200) will be selected when both paths are available

### STATIC_ROUTE - SR-Policy Color Attribute

The STATIC_ROUTE table supports an optional `color` attribute to enable SR-Policy based routing for static routes.

**Key:** vrf_name|ip_prefix (e.g., "default|10.1.0.0/24")

**New Attribute:**
- **color**: Comma-separated list of SR-Policy color values (0-4294967295) corresponding to each nexthop. Default is "0" (no SR-Policy). Value 0 means no SR-Policy is associated with that nexthop. When a non-zero color (1-4294967295) is specified for a nexthop, the static route will use the SR-Policy matching that color and the nexthop IP as the endpoint.

**Example - Static Route with SR-Policy:**
```json
{
    "STATIC_ROUTE": {
        "default|10.1.0.0/24": {
...
In this example:
- First nexthop (192.168.1.1) uses SR-Policy with color 100 and endpoint 192.168.1.1
- Second nexthop (192.168.1.2) uses regular routing (color 0)

**Routing Configuration Generated:**
```
ip route 10.1.0.0/24 192.168.1.1 color 100
ip route 10.1.0.0/24 192.168.1.2
```
...
ip route 10.2.0.0/24 192.168.1.3 30
```

**Backward Compatibility:**
- If `color` is not specified, it defaults to "0" for all nexthops (no SR-Policy)
- Existing configurations without `color` continue to work unchanged
- The number of color values must match the number of nexthops
...
>>>>>>> 3f69313c0 (NOS-13854: Adding a 'must' statement to disallow sharing of segment-lists between paths/policies (#7834))
