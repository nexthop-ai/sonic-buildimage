...
class TestBuildCmds:
    ...
    def test_gr_enable(self):
        assert BgpGlobalsMgr._build_gr_enable_cmds("65001") == [
            "router bgp 65001",
            " bgp graceful-restart",
            "exit",
        ]

    def test_gr_disable(self):
        assert BgpGlobalsMgr._build_gr_disable_cmds("65001") == [
            "router bgp 65001",
            " no bgp graceful-restart",
            "exit",
        ]

    def test_graceful_shutdown_enable(self):
        assert BgpGlobalsMgr._build_graceful_shutdown_enable_cmds("65001") == [
            "router bgp 65001",
            " bgp graceful-shutdown",
            "exit",
        ]

    def test_graceful_shutdown_disable(self):
        assert BgpGlobalsMgr._build_graceful_shutdown_disable_cmds("65001") == [
            "router bgp 65001",
...
class TestSetHandler:
    ...
    def test_graceful_restart_enable(self):
        mgr = make_mgr()
        run_set(mgr, "default", {"graceful_restart_enable": "true"},
                expect_cmds=[LLGR_STALE_TIME_DISABLE, SELECT_DEFER_TIME_DISABLE,
                             GR_ENABLE, GSHUT_DISABLE])
        assert mgr._gr_active is True
        assert mgr._graceful_shutdown_active is False

    def test_graceful_shutdown_enable(self):
        mgr = make_mgr()
        run_set(mgr, "default", {"graceful_shutdown": "true"},
                expect_cmds=[LLGR_STALE_TIME_DISABLE, SELECT_DEFER_TIME_DISABLE,
                             GR_DISABLE, GSHUT_ENABLE])
        assert mgr._gr_active is False
        assert mgr._graceful_shutdown_active is True

    def test_graceful_knobs_together(self):
        mgr = make_mgr()
        run_set(mgr, "default",
                {"graceful_restart_enable": "true", "graceful_shutdown": "true"},
                expect_cmds=[LLGR_STALE_TIME_DISABLE, SELECT_DEFER_TIME_DISABLE,
                             GR_ENABLE, GSHUT_ENABLE])
        assert mgr._gr_active is True
        assert mgr._graceful_shutdown_active is True

    def test_graceful_explicit_false_disables(self):
        # These are booleans, so 'false' and absence are the same desired state.
        mgr = make_mgr()
        run_set(mgr, "default",
                {"graceful_restart_enable": "false", "graceful_shutdown": "false"},
                expect_cmds=[LLGR_STALE_TIME_DISABLE, SELECT_DEFER_TIME_DISABLE,
                             GR_DISABLE, GSHUT_DISABLE])
        assert mgr._gr_active is False
        assert mgr._graceful_shutdown_active is False

    def test_graceful_undrain_after_drain(self):
        # The operational drain → undrain cycle the ticket motivates.
        mgr = make_mgr()
        run_set(mgr, "default", {"graceful_shutdown": "true"})
        assert mgr._graceful_shutdown_active is True
        run_set(mgr, "default", {},
    ...
    def test_graceful_mixed_case_true(self):
        mgr = make_mgr()
        run_set(mgr, "default", {"graceful_shutdown": "True"},
                expect_cmds=[LLGR_STALE_TIME_DISABLE, SELECT_DEFER_TIME_DISABLE,
                             GR_DISABLE, GSHUT_ENABLE])
...
class TestDelHandler:
    ...
    def test_del_when_both_active_pushes_both_disables(self):
        ...
        run_set(mgr, "default",
                {"llgr_stale_time": "3600", "gr_select_defer_time": "240"})
        # Only the two knobs that were actually activated get a disable on
        # delete — graceful-restart / graceful-shutdown were never enabled.
        run_del(mgr, "default", expect_cmds=[LLGR_STALE_TIME_DISABLE, SELECT_DEFER_TIME_DISABLE])
        assert mgr._llgr_active is False
        assert mgr._select_defer_active is False

    def test_del_when_graceful_active_pushes_disables(self):
        mgr = make_mgr()
        run_set(mgr, "default",
                {"graceful_restart_enable": "true", "graceful_shutdown": "true"})
        run_del(mgr, "default", expect_cmds=[GR_DISABLE, GSHUT_DISABLE])
        assert mgr._gr_active is False
        assert mgr._graceful_shutdown_active is False
